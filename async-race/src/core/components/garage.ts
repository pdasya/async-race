...
class Garage extends Component {
    ...
    toggleCarButtons(id: string, option: string): void {
    ...
    toggleUpdateButton(option: string) {
    ...
    toggleCreateButton(option: string) {
    ...
    togglePaginationButtons(option: string) {
    ...
    toggleAllButtons(option: string, id: string) {
        if (option === 'enable') {
    ...
    togglePageButtons(variant: string) {
        ...
        if (!(navButtonWinner instanceof HTMLAnchorElement))
            throw new Error('NavButtonWinner is not HTMLAnchorElement');
        if (variant === 'enable') {
        ...
        await Promise.all(
            ...
            cars.forEach(async (car) => {
                ...
                if (isFinished && isNotFinished && stop !== 'broken') {
                    ...
                    if (curCar.id) {
                        if (!curCar.wins || curCar.time === undefined)
                            throw new Error('Car wins or time is not defined');
    ...
    toggleRaceResetButtons(option: string) {
